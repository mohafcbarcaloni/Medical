import { createFileRoute, useNavigate, Link } from "@tanstack/react-router"; // update to trigger regeneration
import { useState } from "react";
import { motion } from "framer-motion";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { User, Mail, Lock, Eye, EyeOff, Stethoscope, ArrowRight, Github, Chrome as Google, Facebook } from "lucide-react";
import { toast } from "sonner";
import { signupFn, socialLoginFn } from "@/lib/auth";
import { Scene3D } from "@/components/Scene3D";

export const Route = createFileRoute("/signup")({
  head: () => ({
    meta: [{ title: "Create Account — Dr. Abdellatif Tarek" }],
  }),
  component: Signup,
});
...
function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await (signupFn as any)({ data: { name: form.name, email: form.email, password: form.password } });
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#0a0a0a] flex items-center justify-center p-6">
      ...
      {/* Dynamic Glows */}
      <div className="absolute right-1/4 top-1/4 h-[500px] w-[500px] rounded-full bg-primary/10 blur-[120px] animate-pulse pointer-events-none" />
      <div className="absolute left-1/4 bottom-1/4 h-[400px] w-[400px] rounded-full bg-accent/10 blur-[100px] animate-float pointer-events-none" />
      ...
      >
        <div className="glass-dark rounded-[2.5rem] border border-white/10 p-8 md:p-12 shadow-elegant backdrop-blur-3xl">
          {/* Header */}
          <div className="mb-10 text-center">
            ...
            <h1 className="font-display text-4xl font-bold text-white tracking-tight">Join the Practice</h1>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/30 group-focus-within:text-accent transition-colors" />
                  <Input
                    value={form.name}
          ...
          <div className="mt-10">
            <div className="relative mb-8">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-[#121620] px-4 text-white/40 tracking-widest">Or sign up with</span>
              </div>
            </div>
          ...
          <p className="mt-10 text-center text-white/40">
            ...
            <Link to="/login" className="text-accent font-semibold hover:underline decoration-accent/30 underline-offset-4">
              Log in instead
            </Link>
          </p>
        </div>
